...
#Combine Relative Abundance & Metadata into One Table
hhra0 = left_join(ra2_0S, hh, by = "Sample")

#Organize Species in rows by decreasing abundance
...
anno$Groups <- factor(anno$Groups, levels = c("ND", "UTI", "BV", "BVUTI"))

#Graphing the Heatmap
library(pheatmap)
library(RColorBrewer)

pheatmap(CSTclust_0sortNA50, cluster_rows = FALSE, cluster_cols = FALSE, show_rownames = FALSE, show_colnames = TRUE, 
angle_col = 90, fontsize_col = 7, main = "Evvy Participants Clustered by CST (SAGE2.2, 0% Threshold)", 
fontsize = 6, color = brewer.pal(9,"PuRd"), annotation_row = anno, annotation_colors = anno_color)
