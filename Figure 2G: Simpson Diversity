...
library(vegan)
ra2_0S$SimpsonD = vegan::diversity(ra2_0, index = "simpson")

library(ggplot2)
library(ggbeeswarm)
library(ggpubr)
ggplot(ra2_0S, aes(Groups,SimpsonD)) + 
  geom_boxplot(aes(fill = Groups), lwd = 0.5, outlier.shape = NA, alpha = 0.5) + 
  labs(title = "Simpson Diversity", x = NULL) + 
...
theme(panel.grid = element_blank(),
  ...
  scale_y_continuous(name = "Simpson Index", limits = c(0, 1.7*max(ra2_0S$SimpsonD))) + scale_fill_manual(
