...
# Default mapping based on category (numeric prefix like "1_" or "2_" is ignored)
...
def strip_numeric_prefix(label):
    # "1_BV" -> "BV", "2_BV&UTI" -> "BV&UTI"
    return re.sub(r'^\d+_', '', str(label))

def parse_class_colors_arg(s):
    """
    Parse: "BV=#F8766D,ND=#00BFC4,UTI=#C77CFF,BV&UTI=#7CAE00"
    """
    cmap = {}
    if not s:
        return cmap
...
def display_class(label, strip_prefix=True):
    return strip_numeric_prefix(label) if strip_prefix else str(label)

def color_for_class(label, params, indcl=None, cls2=None):
    # Prefer explicit mapping if available
    dlab = display_class(label, params.get('strip_class_prefix', True))
    cmap = params.get('class_color_map', {})
    if dlab in cmap:
        return cmap[dlab]

    # Fall back to existing behavior
    if cls2 is not None and len(cls2) > 0:
        return colors[cls2.index(label) % len(colors)]
    if indcl is None:
        return colors[0]
    return colors[indcl % len(colors)]
...
def read_params(args):
    ...
    parser.add_argument('--feature_font_size', dest="feature_font_size", type=int, default=9, help="the file for the output image")
    ...
    # New: configurable class colors + prefix stripping
    parser.add_argument('--class_colors', dest="class_colors", type=str, default="",
                        help='Override class color mapping, e.g. "BV=#F8766D,ND=#00BFC4,UTI=#C77CFF,BV&UTI=#7CAE00"')
    parser.add_argument('--strip_class_prefix', dest="strip_class_prefix", default=True, action='store_true',
                        help='Strip leading "<number>_" from class labels for color mapping/legend (default)')
    parser.add_argument('--no_strip_class_prefix', dest="strip_class_prefix", action='store_false',
                        help='Do not strip numeric prefixes from class labels')
...
def plot_histo_hor(path,params,data,bcl,report_features):
    ...
    for i,v in enumerate(data['rows']):
        if report_features:
            ...
            score = v[3]
            otu_class = display_class(v[2], params.get('strip_class_prefix', True))
            out_data[otu] = [score, otu_class]
        indcl = cls.index(v[2])

        dlab = display_class(v[2], params.get('strip_class_prefix', True))
        lab = dlab if dlab not in added else ""
        added.append(dlab)

        col = color_for_class(v[2], params, indcl=indcl, cls2=cls2)
...
def plot_histo_ver(path,params,data,report_features):
    ...
    for i,v in enumerate(data['rows']):
        ...
        col = color_for_class(v[2], params, indcl=indcl)
...
if __name__ == '__main__':
    ...
    # Initialize default class color map; allow CLI override
    params['class_color_map'] = dict(DEFAULT_CLASS_COLORS)
    params['class_color_map'].update(parse_class_colors_arg(params.get('class_colors', "")))
